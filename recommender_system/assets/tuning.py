...
@asset(
    ...
    required_resource_keys={"mlflow_tuning"},
    group_name="tuning"
)
def tuning_baseline_model_best_param(
    ...
    mlflow = context.resources.mlflow_tuning
    ...
    for i, p in enumerate(ps):
        with mlflow.start_run():
            ...
            md = calc_metrics(y_train, y_pred, prefix="train")
            mlflow.log_metrics(md)

            y_pred = model.predict(X_val)
            md = calc_metrics(y_val, y_pred, prefix="val")
            mlflow.log_metrics(md)
